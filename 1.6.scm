...
;
; При нормальном порядке вычисления всё будет работать как и прежде.
...
; нормального порядка такое поведение не наблюдается, поскольку аргументы вычисляются после подстановки в тело процедуры, т.е.
; следующий шаг рекурсии будет вычисляться как и для if при необходимости.
;
; P.S. Заданиие дискриминирует женщин
